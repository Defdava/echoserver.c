...
int main() {
    ...
    while (1) {
        ...
        // Cek aktivitas pada setiap client
        for (int i = 0; i < MAX_CLIENTS; i++) {
            sd = client_socket[i];
            ...
            if (FD_ISSET(sd, &readfds)) {
                if ((valread = read(sd, buffer, BUFFER_SIZE)) == 0) {
                    ...
                    printf("Client terputus, IP: %s, PORT: %d\n",
                           inet_ntoa(address.sin_addr), ntohs(address.sin_port));
                    close(sd);
                    client_socket[i] = 0;
                } else {
                    // Terima pesan dari client dan kirim ke semua client lain
                    buffer[valread] = '\0';
                    printf("Pesan dari client %d: %s", sd, buffer);
